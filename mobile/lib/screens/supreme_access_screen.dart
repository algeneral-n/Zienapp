...
class SupremeAccessScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SupremeAccessScreen> createState() =>
      _SupremeAccessScreenState();
}
...
class _SupremeAccessScreenState extends ConsumerState<SupremeAccessScreen>
  ...
  final List<Map<String, dynamic>> _cmdHistory = [];
  ...
  Future<void> _loadAuditLog() async {
    ...
    if (res.isSuccess && res.data != null) {
      setState(
        () =>
            _auditLog = List<Map<String, dynamic>>.from(
              res.data?['logs'] ?? [],
            ),
      );
    }
  ...
  Future<void> _loadQueue() async {
    ...
    if (res.isSuccess && res.data != null) {
      setState(
        () =>
            _queueCommands = List<Map<String, dynamic>>.from(
              res.data?['commands'] ?? [],
            ),
      );
    }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: Text(
              'This will execute the AI-requested command.',
  ...
  Future<void> _rejectQueueItem(String id) async {
    await _api.post(
      '/api/supreme/queue/$id/reject',
      body: {'reason': 'Founder rejected'},
    );
  ...
  Future<void> _executeCommand() async {
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  'Command Type: ${_cmdTypes[_cmdType]?['label'] ?? _cmdType}',
    ...
    final res = await _api.post(
      '/api/supreme/execute',
      body: {'command': command, 'type': _cmdType},
    );
              ...
              builder:
                  (ctx, scrollCtrl) => ListView(
                    ...
                    children: [
                      ...
                      ...((res.data?['sections'] as List<dynamic>?) ?? [])
                          .map<Widget>((section) {
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Supreme Access is restricted to platform founder.',
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    'End-to-end encrypted • Founder only',
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (_auditLog.isEmpty) _emptyState('No activity logs yet'),
        ],
      ),
    );
  }
  ...
  Widget _metricsGrid() {
    final items = <_MetricItem>[
      _MetricItem(
        'Active Tenants',
        '${_metrics['active_tenants'] ?? _health['tenants'] ?? '--'}',
        Icons.business,
        Colors.blue,
      ),
      _MetricItem(
        'Total Users',
        '${_metrics['total_users'] ?? _health['users'] ?? '--'}',
        Icons.people,
        Colors.teal,
      ),
      _MetricItem(
        'AI Queries (24h)',
        '${_metrics['ai_queries_24h'] ?? '--'}',
        Icons.psychology,
        Colors.purple,
      ),
      _MetricItem(
        'Revenue (MTD)',
        '\$${_metrics['revenue_mtd'] ?? '--'}',
        Icons.attach_money,
        Colors.green,
      ),
      _MetricItem(
        'Active Sessions',
        '${_metrics['active_sessions'] ?? '--'}',
        Icons.devices,
        Colors.orange,
      ),
      _MetricItem(
  ...
  Widget _buildSecurityTab() {
    return ListView(
      ...
      children: [
        ...
        if (_auditLog.isEmpty) _emptyState('No audit entries'),
      ],
    );
  }
  ...
  Widget _threatLevel() {
    final level =
        _alerts.isEmpty ? 'LOW' : (_alerts.length < 3 ? 'MEDIUM' : 'HIGH');
    final color =
        level == 'LOW'
            ? Colors.green
  ...
  Future<void> _confirmSecurityAction(String action) async {
    ...
    if (confirmed == true) {
      ...
      final res = await _api.post(
        '/api/supreme/execute',
        body: {'command': action, 'type': 'security'},
      );
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor:
                res.isSuccess ? Colors.green[800] : Colors.red[800],
          ),
        );
        _loadAllData();
      }
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'AI-powered reports generated on-demand',
                      style: TextStyle(color: Colors.grey[400], fontSize: 12),
                    ),
                  ],
                ),
                  ...
                  ? Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Execute platform commands with confirmation',
                          style: TextStyle(
                            color: Colors.grey[500],
                            fontSize: 12,
                          ),
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  (_cmdTypes[cmd['type']]?['label'] as String?) ??
                      cmd['type'] as String? ??
                      '',
                  style: const TextStyle(
                    color: Colors.amber,
      ...
      child: ListView(
        ...
        children: [
          ...
          _capabilityTile(
            ...
            Icons.monitor_heart,
            Colors.blue,
            'Read-only access to metrics and health',
          ),
          _capabilityTile(
            'Security Scanning',
            Icons.security,
            Colors.red,
            'Detect threats and anomalies autonomously',
          ),
          _capabilityTile(
            'Generate Reports',
            Icons.assessment,
          ...
          _capabilityTile(
            ...
            Icons.queue,
            Colors.orange,
            'Queue commands for your approval',
          ),
          _capabilityTile(
            'Heartbeat Check-in',
            Icons.favorite,
            Colors.pink,
            'Periodic alive signal with metrics',
          ),
        ],
      ),
    );
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  cmd['command_type'] as String? ?? 'query',
                  style: const TextStyle(
                    color: Colors.amber,
                    fontSize: 9,
                    fontWeight: FontWeight.bold,
