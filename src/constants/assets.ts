/**
 * ZIEN Platform Asset Constants
 * Using direct paths for system-provided images.
 * These can be replaced with Base64 strings for full embedding.
 */

export const ASSETS = {
  // Logos
  LOGO_PRIMARY: "/zien-logo.png",
  LOGO_ICON: "/favicon.jpg",
  LOGO_SVG: "/zien-platform-logo.svg",
  LOGO_SHIELD: "/icons/app-icon.jpg",

  // Splash & Video
  SPLASH: "/splash.gif",
  VIDEO_EN: "https://drive.google.com/file/d/1Co_CZAC37_deqopDsAnn_0XWPD0dbP5q/preview",
  VIDEO_AR: "https://drive.google.com/file/d/1cp8_vZiGbUcjFLJIPomYJNfT2DYODct9/preview",

  // Characters
  RARE_AGENT: "https://lh3.googleusercontent.com/p/AF1QipN2YjssfAFq4DmfPDprA9w13UVqNyEXmnkrGR0i=w141-h235-n-k-no-nu",
  RARE_CHARACTER: "https://lh3.googleusercontent.com/p/AF1QipN2YjssfAFq4DmfPDprA9w13UVqNyEXmnkrGR0i=w141-h235-n-k-no-nu",
  RARE_CHARACTER_FULL: "/rare-character.png",

  // Dashboard & Features
  DASHBOARD_INVOICES: "https://lh3.googleusercontent.com/p/AF1QipMTWzrywgzSxXWwjgPJ9k1OsOWNiBCKmd00adb4=w141-h118-n-k-no-nu",
  USER_MANAGEMENT: "https://lh3.googleusercontent.com/p/AF1QipOYNgk4T03rnwxTL1hMHg6laY7EQ0WGbMaIBXfw=w141-h141-n-k-no-nu",
  DOC_GENERATION: "https://lh3.googleusercontent.com/p/AF1QipNustYNEWFsvJGQC3s1kmSVFruhjGak-2pSNs2F=w141-h141-n-k-no-nu",
  MODULES_OVERVIEW: "https://lh3.googleusercontent.com/p/AF1QipOO-DdmdlmqYyUXb1rL9T8Bv880KvuLBJr9uJHa=w141-h176-n-k-no-nu",

  // Landing Page Carousel
  LANDING_1: "https://lh3.googleusercontent.com/p/AF1QipMvY2-IPoxLaToodwo7egfEuzhKGkjOX1jxceA_=w243-h304-n-k-no-nu",
  LANDING_2: "https://lh3.googleusercontent.com/p/AF1QipNmr43mTEUk82q4c3EwAyP_Rqet8Mtxyl1STbJY=w243-h174-n-k-no-nu",

  // Feature Images
  FEATURE_ANALYTICS: "https://lh3.googleusercontent.com/jpOJB2YOWH9eQkV8kt6TDwm2-mbOB5BcjFQHjhPoAbyxoKKyB60Vmd11WkfIN5qGTVhxSeFdhDgEAaxlZg=s265-w265-h265",
  FEATURE_COMMUNICATION: "https://lh3.googleusercontent.com/s00PvAlcfvwqlDKgH__hMv5W1XUe6yAXzqXmDqrqrg8eOFQS2IajShh7v4ovjrDIE14cEI2C_56mS4vpQQ=s265-w265-h265",
  FEATURE_SECURITY_ALT: "/landing images/security.png",
  FEATURE_AUTOMATION: "https://lh3.googleusercontent.com/P5zn4Y2m_bu2XfVrPPrPyY9yT7v3yPSaheYwJWOSZadN_J9CIbV0i4NXiRueUB8-WE76UEu9BFO_1k00tQ=s265-w265-h265",
};

export const IMAGE_PROPS = {
  referrerPolicy: "no-referrer" as const,
};
