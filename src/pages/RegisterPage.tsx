...
const COMPANY_TYPES = [
  ...
  { id: 'trading', name: 'Trading Company', icon: Briefcase, color: 'bg-emerald-500' },
          ...
          <div className="hidden md:flex items-center gap-4">
            {[1, 2, 3, 4].map((s) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${step >= s ? 'bg-emerald-500 text-white' : 'bg-zinc-200 dark:bg-zinc-800 text-zinc-500'}`}>
                  {step > s ? <CheckCircle2 size={16} /> : s}
                </div>
                {s < 4 && <div className={`w-8 h-0.5 rounded-full ${step > s ? 'bg-emerald-500' : 'bg-zinc-200 dark:bg-zinc-800'}`} />}
              </div>
            ))}
          </div>
          <HeaderControls />
          ...
          <AnimatePresence mode="wait">
              ...
              >
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input 
                          ...
                          className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl py-4 pl-12 pr-4 focus:ring-2 focus:ring-emerald-500 transition-all text-sm font-medium"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {COMPANY_TYPES.map((type) => (
                          <button
                            ...
                            className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 text-center ${formData.companyType === type.id ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'border-zinc-100 dark:border-zinc-800 hover:border-zinc-200'}`}
                          >
                ...
                <div className="mt-10 flex justify-end">
                  <button 
                    ...
                    className="bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 text-white px-10 py-4 rounded-2xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-emerald-500/20"
                  >
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full bg-zinc-50 dark:bg-zinc-800 border-none rounded-2xl py-4 px-4 focus:ring-2 focus:ring-emerald-500 transition-all text-sm font-medium"
            ...
            {step === 3 && (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl p-10 text-center hover:border-emerald-500 transition-colors cursor-pointer group">
                    <div className="w-16 h-16 bg-zinc-100 dark:bg-zinc-800 rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:bg-emerald-500 group-hover:text-white transition-all">
                      <Upload size={24} />
                    </div>
                ...
                <div className="mt-10 p-6 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl">
                  <label className="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" className="mt-1 rounded border-zinc-300 text-emerald-500 focus:ring-emerald-500" />
                    <span className="text-xs text-zinc-500 font-medium leading-relaxed">
                      I hereby confirm that all provided information is accurate and I accept ZIEN's <a href="#" className="text-emerald-500 font-bold hover:underline">Terms of Service</a>, <a href="#" className="text-emerald-500 font-bold hover:underline">Privacy Policy</a>, and <a href="#" className="text-emerald-500 font-bold hover:underline">Data Processing Agreement</a>.
                    </span>
                  </label>
                </div>
                ...
                <div className="mt-10 flex justify-between">
                  ...
                  <button 
                    onClick={nextStep}
                    className="bg-emerald-500 hover:bg-emerald-600 text-white px-10 py-4 rounded-2xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-emerald-500/20"
                  >
                    Next Step <ArrowRight size={18} />
                  </button>
                </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {PLANS.map((plan) => (
                    <button
                      ...
                      className={`p-8 rounded-[32px] border-2 transition-all text-left flex flex-col ${formData.plan === plan.id ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'border-zinc-100 dark:border-zinc-800 hover:border-zinc-200'}`}
                    >
                      ...
                      <ul className="space-y-3 mb-8 flex-1">
                        <li className="text-xs font-bold text-zinc-500 flex items-center gap-2">
                          <CheckCircle2 size={14} className="text-emerald-500" /> Up to {plan.users} Users
                        </li>
                        <li className="text-xs font-bold text-zinc-500 flex items-center gap-2">
                          <CheckCircle2 size={14} className="text-emerald-500" /> {plan.features}
                        </li>
                        <li className="text-xs font-bold text-zinc-500 flex items-center gap-2">
                          <CheckCircle2 size={14} className="text-emerald-500" /> RARE AI Basic
                        </li>
                      </ul>
                      <div className={`w-full py-3 rounded-xl text-center text-xs font-bold uppercase tracking-widest transition-all ${formData.plan === plan.id ? 'bg-emerald-500 text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500'}`}>
                ...
                <div className="mt-10 flex justify-between items-center">
                  ...
                  <button 
                    ...
                    className="bg-emerald-500 hover:bg-emerald-600 text-white px-12 py-4 rounded-2xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-emerald-500/20"
                  >
            ...
            {step === 5 && (
              ...
              >
                <div className="w-24 h-24 bg-emerald-500 rounded-3xl flex items-center justify-center text-white mx-auto mb-8 shadow-2xl shadow-emerald-500/20">
                  <motion.div
                    animate={{ rotate: 360 }}
                ...
                <p className="text-xl text-zinc-500 font-medium mb-8">
                  Configuring modules for <span className="text-emerald-500 font-bold">{formData.companyName}</span> based on <span className="text-emerald-500 font-bold">{formData.companyType}</span> template.
                </p>
